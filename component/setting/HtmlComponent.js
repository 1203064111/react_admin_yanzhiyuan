    ...
    render(){
        return(
            <div style={{background:'#ffffff',display:'flex',flexDirection:'column'}}>
                ...
                <Widget.Toolbar title="政策协议列表" history={this.props.history}></Widget.Toolbar>
                <div style={{display:'flex',justifyContent:'flex-end',marginTop:20}}>
                    <Widget.Button
                        marginRight={20}
                        value="添加"
                        onClick={()=>{
                            this.props.history.push("/html_editor/-1");
                        }}/>
                </div>
                <Widget.List
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/html_editor/"+this.state.htmlBeans[rowID].html_id);
                                break;
                            case 1:
                                this.setState({
                                    visible:true,
