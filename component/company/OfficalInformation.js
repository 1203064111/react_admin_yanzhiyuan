...
class Information extends Widget.Base{//掌牧资讯
    ...
    getInformations(){
        this.getDataByPost(1,information_homeUrl+"/informationController/v1.0/getSystemInformations"
            ,{page:this.state.page,information_state:this.state.information_state,class_id:2},{type:2})
    }
    deleteInformation(){
    ...
    render(){
        return(
            <div style={{background:'#ffffff',display:'flex',flexDirection:'column'}}>
                <Widget.Toolbar title={"官方资讯列表"} history={this.props.history}></Widget.Toolbar>
                <Widget.View>
                    <Widget.Button
                        marginLeft={20}
                        value="添加"
                        onClick={()=>{
                            this.props.history.push("/offical_information_editor/-1");
                        }}/>
                </Widget.View>
                ...
                <Widget.List
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/offical_information_editor/"+this.state.InformationBeans[rowID].information_id);
                                break;
                            case 1:
                                this.openTip("确认删除?",()=>{
                                    this.setState({
