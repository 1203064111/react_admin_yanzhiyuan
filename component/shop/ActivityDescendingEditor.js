...
class ActivityDescendingEditor extends Widget.Base{
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            activityBean:{activity_type:"3"},
            activityDescendingBean:{}
        };
    }

    componentDidMount() {
        if(this.props.params.activity_id!=="-1"){
            this.getActivityDescendingDetail();
        }
    }

    getActivityDescendingDetail(){
        this.getDataByPost(1,shop_homeurl+"/shopActivityController/v1.0/getActivityDescendingDetail",{activity_id:this.state.activity_id});
    }
    ...
    doSuccess(index,data){
        switch (index){
            case 1:
                this.setState({
                    activityBean:data,
                    activityDescendingBean:data.activityDescendingBean
                })
                break;
            case 2:
                this.showTip("添加成功");
    ...
    insertActivity(){
        ...
        params["descending_id"]=this.state.activityDescendingBean.descending_id;
        params["descending_price"]=this.state.activityDescendingBean.descending_price;
        ...
        return(
            <div>
                ...
                <Widget.Detail
                    title="优惠信息"
                    baseData={[
                        {name:"直降价格",flex:1,key:'descending_price'},
                    ]}
                    data={this.state.activityDescendingBean}
                    onChange={(key,value,index)=>{
                        this.state.activityDescendingBean[key]=value;
                        this.refresh();
                    }}/>
            </div>
        )
...
module.exports=ActivityDescendingEditor;