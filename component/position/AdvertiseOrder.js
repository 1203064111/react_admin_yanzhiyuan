...
class AdvertiseOrder extends Widget.Base{
    constructor(props) {
        ...
        this.state = {
            ...
            payTypeBeans:[{name:"全部",value:''},
                {name:"线上",value:'0'},
                {name:"线下",value:'1'}],
            order_type:this.getNull(this.getSessionStorage(type+"order_type"),1),
    ...
    exportOrders(){
        this.getDataByPost(3,shop_homeurl+"/orderController/v1.0/exportAdvertiseOrders"
            ,{order_type:this.state.order_type,
                order_state:this.state.order_state,
                order_no:this.state.order_no,
                pay_type:this.state.pay_type})
    ...
    render(){
        ...
        return(
            <div>
                <Widget.Toolbar title={"定金订单列表"} history={this.props.history}></Widget.Toolbar>
                <Widget.View>
                    <Widget.Select
                        width={60}
                        selectHeight={30}
                ...
                <Widget.List
                    data={[{name:"订单ID",flex:1,key:'advertise_order_id'},
                        ...
                        {name:"收款",flex:1,key:'order_pay_first'},
                        {name:"押金",flex:1,key:'advertise_order_total'},
                        {name:"是否打款给商家",flex:1,key:'order_remit',type:'radio_select'},
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/advertise_order_detail/"+this.state.advertiseOrderBean[rowID].advertise_order_id);
                                break;
                            case 1:
                                this.setState({
                                    visible:true,
                             ...
                             if(value==="1"){
                                 this.openTip("确认打款?",()=>{
                                     this.setState({
                                         ...
                                         this.getDataByPost(2,information_homeUrl+"/positionController/v1.0/updateAdvertiseOrder"
                                             ,{advertise_order_id:this.state.advertiseOrderBean[rowID].advertise_order_id,
                                                 order_remit:value});
                                     })
                                 })
