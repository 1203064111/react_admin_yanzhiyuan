...
class RenovationOrder extends Widget.Base{
    constructor(props) {
        super(props);
        // 初始状态
        this.state = {
    ...
    sendCustomizedOrder(){
        this.getDataByPost(2,maintail_homeurl+"/maintailOrderController/v1.0/sendRenovationOrder",{order_id:this.state.order_id})
    }
    cancelRenovationOrder(){
        this.getDataByPost(3,maintail_homeurl+"/maintailOrderController/v1.0/cancelRenovationOrder",{order_id:this.state.order_id})
    }
    getOrderMerchants(){
        this.getDataByPost(1,maintail_homeurl+"/maintailOrderController/v1.0/getRenovationOrders",
    ...
    render(){
        return(
            <div>
                <Widget.Tip visible={this.state.visible} msg="确认订单?"
                            onClose={()=>{
                                this.setState({
                                    visible:false
                                })
                ...
                <Widget.View>
                    ...
                    <Widget.Select
                        ...
                        onChange={(index)=>{
                            this.setState({
                                order_type:this.state.typeBeans[index].value
                            })
                        }}/>
                    <Widget.Editor
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                ...
                                break;
                            case 1:
                                this.props.history.push("/renovation_order_editor/"+this.state.orderMerchantsBeans[rowID].order_id);
                                break;
                                break;
                            case 2:
...
module.exports=RenovationOrder;
