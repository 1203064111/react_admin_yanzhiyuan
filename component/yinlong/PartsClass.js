...
class PartsClass extends Widget.Base{
    constructor(props) {
        super(props);
        // 初始状态
        this.state = {
    ...
    exportPratsClassesV1(){
        this.getDataByPost(5,maintail_homeurl+"/pratsClassController/v1.0/exportPratsClassesV1")
    }
    doSuccess(index,data){
        switch (index){
            case 1:
                this.setState({
                    pratsClassBeans:data.data,
                    total:data.total
                });
                break;
            case 2:
        ...
        return(
            <div style={{background:'#ffffff',display:'flex',flexDirection:'column'}}>
                ...
                <Widget.List
                    ...
                    dataSource={this.state.pratsClassBeans}
                    operationData={[{title:"二级分类"},{title:"编辑"},{title:"删除"}]}
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/parts_class2/"+this.state.pratsClassBeans[rowID].parts_id);
                                break;
                            case 1:
                                this.props.history.push("/parts_class_editor/"+this.state.pratsClassBeans[rowID].parts_id+"/"+this.state.parent_id);
                                break;
                            case 2:
                                this.setState({
                                    visible:true,
                                    parts_id:this.state.pratsClassBeans[rowID].parts_id
                                })
                                break;
                        }
                    }}
                    onChange={(rowID,key,value)=> {
                        if(key==="sort"){
                            this.getDataByPost(4,maintail_homeurl+"/pratsClassController/v1.0/movePratsClass"
                                ,{parts_id:this.state.pratsClassBeans[rowID].parts_id
                                   , parent_id:this.state.pratsClassBeans[rowID].parent_id
                                    ,sort:this.state.pratsClassBeans[rowID].sort,
                                    sort_type:value})
                        }else if(key==="parts_state"){
                            console.log(this.state.pratsClassBeans[rowID].parts_id+"-----"+value);
                            this.getDataByPost(3,maintail_homeurl+"/pratsClassController/v1.0/updatePratsClass",
                                {parts_id:this.state.pratsClassBeans[rowID].parts_id,
                                    parts_state:value});
                        }
                        this.state.pratsClassBeans[rowID][key]=value;
                        this.refresh();
