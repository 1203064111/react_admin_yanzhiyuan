...
class SceneGiveRefuse extends Widget.Base{
    constructor(props) {
        super(props);
        // 初始状态
    ...
    getScenes(){
        this.getDataByPost(1,koubei_homeurl+"/sceneController/v1.0/getScenes"
            ,{page:this.state.page,scene_type:this.state.scene_type,
    ...
    render(){
        return(
            <div style={{background:'#ffffff',display:'flex',flexDirection:'column'}}>
                <Widget.Toolbar title={"满赠场景"} history={this.props.history}></Widget.Toolbar>
                <Widget.View>
                    <Widget.Button
                        marginLeft={20}
                        value="添加"
                        onClick={()=>{
                            this.props.history.push("/scene_give_editor/-1");
                        }}/>
                </Widget.View>
                <Widget.List
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/scene_give_editor/"+this.state.sceneBeans[rowID].scene_id);
                                break;
                            case 1:
                                this.openTip("确认删除?",()=>{
...
module.exports=SceneGiveRefuse;